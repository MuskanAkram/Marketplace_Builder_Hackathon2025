    ...
    <div className="bg-white h-[60px] md:h-[70px] w-full flex items-center justify-between px-4 sm:px-6 md:px-10 fixed top-0 left-0 z-50 shadow-lg transition-all duration-300">
      <div className="hidden md:flex items-center justify-between w-full space-x-10">
        <Link href="/" className="flex items-center space-x-4">
          ...
          />
          <h2 className="text-black text-[26px] font-extrabold hidden sm:block">
            Furniro
          </h2>
        </Link>
        <div className="flex items-center space-x-8">
          {["Home", "Shop", "Blog", "Contact"].map((link) => (
            <Link href={link === "Home" ? "/" : `/${link}`} key={link} passHref>
              <div className="text-[18px] text-black transition-all duration-300">
                {link}
              </div>
            </Link>
          ))}
      ...
      <div className="md:hidden flex items-center justify-between w-full">
        ...
        <Link href="/" className="flex items-center justify-center flex-grow">
          ...
          />
          <h2 className="text-black text-[22px] sm:text-[24px] md:text-[26px] font-extrabold ml-4 transition-all duration-300">
            Furniro
          </h2>
        </Link>
      ...
      <div
        className={`md:hidden fixed inset-y-0 left-0 bg-white py-4 px-6 shadow-lg z-50 transition-transform duration-300 ${
        ...
        <div className="flex justify-between items-center mb-4">
          <Link href="/" className="flex items-center">
            <Image
              ...
              className="w-[50px] h-[32px] md:w-[60px] md:h-[40px] cursor-pointer"
            />
            <h2 className="text-black text-[22px] sm:text-[24px] md:text-[26px] font-extrabold ml-2">
              Furniro
            </h2>
          </Link>
        ...
        <div className="space-y-4">
          {["Home", "Shop", "Blog", "Contact"].map((link) => (
            <Link href={link === "Home" ? "/" : `/${link}`} key={link} passHref>
              <div
                onClick={closeMenu}
                className="text-[18px] text-black py-2 transition-all duration-300"
              >
                {link}
              </div>
            </Link>
