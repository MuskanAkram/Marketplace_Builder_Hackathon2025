...
const ProductPage = () => {
  ...
  const handleSectionClick = (
    section: "description" | "additional" | "reviews"
  ) => {
    ...
    <div className="max-w-screen-lg mx-auto px-4 py-24 min-h-screen">
      <div className="flex flex-col md:flex-row gap-10 items-start">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="flex-1 flex flex-col justify-between lg:h-full">
            ...
            <h2 className="my-2 text-lg text-gray-900 font-bold">{`$ ${product.price.toFixed(
              2
            )}`}</h2>
            <div className="my-2 flex items-center gap-4">
              <p className="text-sm text-gray-500">
                {product.discountPercentage > 0 && (
                  <span className="bg-[#FFD700] text-black px-2 py-1 rounded-md">
            ...
            <p className="text-gray-700 leading-relaxed my-2">
              {product.description.slice(0, 484)}.....
            </p>
                  ...
                  <div className="flex justify-center mt-2">
                    <div className="flex flex-wrap gap-2">
                      {product.tags.map((tag, index) => (
                        <button
                          key={index}
                          className="px-4 py-2 rounded-md border border-gray-300 hover:bg-[#B88E2F]"
                        >
                          {tag}
                        </button>
                      ))}
      ...
      {activeSection === "description" && (
        <div className="mt-[37px]">
          <p className="text-customGray text-normal">{product.description}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 w-full max-w-screen-xl">
          {products.length > 0 ? (
            products.slice(0, 4).map((product) => (
              <div
                key={product._id}
                className="flex flex-col items-center bg-[#F4F5F7] p-2 relative group rounded-md"
              >
